...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!["New", "Contacted", "Visited", "Qualified", "Purchased"].includes(status)) {
    ...
    try {
      ...
      if (populatedLead && Array.isArray(populatedLead.statusHistory)) {
        ...
        if (last && (typeof last.changedBy === "string" || (last.changedBy && !last.changedBy.name))) {
          ...
          } catch (manualPopError) {
            console.error("[DEBUG] Manual population error:", manualPopError)
          }
        }
      }
      safeLead = JSON.parse(JSON.stringify(populatedLead))
    } catch (popError) {
      console.error("[DEBUG] Population or serialization error:", popError)
      // Fallback: return the lead as plain object
      safeLead = JSON.parse(JSON.stringify(lead))
    }
